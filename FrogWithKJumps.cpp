...
int rec(vector<int>& vec, int i, int n, int k, vector<vector<int>>& dp)
{
    if(i >= n)
        return 1e9;
    if(i == n - 1)
    ...
    {
        ans = rec(vec, i + jumps, n, k, dp);
        if(i + jumps < n)
            best = min(ans + abs(vec[i] - vec[i + jumps]), best);
    }
    return dp[i][k] = best;
}
void solve()
