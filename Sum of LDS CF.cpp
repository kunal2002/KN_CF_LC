...
// Question link : https://codeforces.com/contest/2128/problem/D
#include <bits/stdc++.h>
...
{  
    ...
    int tot_count = (n * (n + 1) * (n + 2)) / 6;
    for(int i = 0; i < n - 1; i++)
    {
        if(vec[i] < vec[i + 1])
        {
            // the elements vec[i] and vec[i + 1] can be present in subarrays starting before or equal to i (length is i + 1 (0-based indexing))
            // and ending at or greater than i + 1 (the length ((n - 1) - (i + 1) + 1)) or (n - (i + 1))
            tot_count -= (i + 1) * (n - (i + 1));
        }
    }
    cout << tot_count << "\n";
