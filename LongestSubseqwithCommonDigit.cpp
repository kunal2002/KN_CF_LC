...
vector<int> res;
int rec(vector<int>& nums, char common, int ind, int n)
{
    if(ind == n)
    {
        cout << res << "\n";
    ...
    string num = to_string(nums[ind]);
    int take = 0, notTake = 0;
    if(common == num[0] || common == '*')
    {
        res.push_back(nums[ind]);
        take = 1 + rec(nums, num[0], ind + 1, n);
        res.pop_back();
    }
    if(common == num[1] || common == '*')
    {
        res.push_back(nums[ind]);
        if(num[0] != num[1])
            take = max(take, 1 + rec(nums, num[1], ind + 1, n));
        res.pop_back();
    }
    notTake = rec(nums, '*', ind + 1, n);
...
int rec(vector<int>& nums, char common, int ind, int n)
{
    ...
    int take = 0, notTake = 0;
    string num = to_string(nums[ind]);
    if(common != num[0] && common != num[1])
    {
        notTake = rec(nums, common, ind + 1, n);
    }
    take = (common == num[0]) + rec(nums, num[0], ind + 1, n);
    if(num[0] != num[1])
        take = max(take, (common == num[1]) + rec(nums, num[1], ind + 1, n));
