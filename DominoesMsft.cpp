...
int dp[50001][7];
int rec(vector<pair<int,int>>& vec, int i, int n, int prev)
{
    if(i == n)
        return 0;
    if(dp[i][prev] != -1)
        return dp[i][prev];
    auto ele = vec[i];
    int notTake = 1e8, Take = 0;
    if(prev == ele.first || prev == 0)
        notTake = rec(vec, i + 1, n, ele.second);
    Take = 1 + rec(vec, i + 1, n, prev);
