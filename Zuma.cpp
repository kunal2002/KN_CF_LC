...
int rec(vector<int>& vec, int l, int r, int n, vector<vector<int>>& dp)
{
    if(l >= r)
        return 1;
    if(dp[l][r] != -1)
            return dp[l][r];
    int res = 1 + rec(vec, l + 1, r, n, dp);
    for(int i = l + 1; i < r; i++)
    {
        if(vec[l] == vec[i])
        res = min(res, rec(vec, l + 1, i - 1, n, dp) + rec(vec, i + 1 , r, n, dp));
    }
    if(vec[l] == vec[r])
        res = min(res, rec(vec, l + 1, r - 1, n, dp));
    return dp[l][r] = res;
}
void solve()
{  
